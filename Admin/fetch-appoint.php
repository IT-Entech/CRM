...
$sqlappoint = "WITH adjusted_data AS (
    ...
    WHERE 
        ...
        " . ($Sales !== 'N' ? "AND A.staff_id = ?" : "") . "
)
SELECT DISTINCT appoint_no, customer_name,
    CASE WHEN qt_no IS NULL THEN '-' ELSE qt_no END AS qt_no,
    format_date, format_qtdate,
    CASE WHEN adjusted_is_pre = 'y' THEN 'pre quotation' ELSE '-' END AS status,
    update_time, remark
FROM adjusted_data
...
$params = $Sales !== 'N' ? [$year_no, $month_no, $Sales] : [$year_no, $month_no];
