...
if ($is_new !== '0' && $is_new_array) {
  $is_new_list = "'" . implode("','", $is_new_array) . "'";
  $where[3][] = "A.customer_type IN ($is_new_list)";
}

if ($year_no !== '0') {
  $where[0][] = "year_no = ?";
  $where[1][] = "YEAR(A.qt_date) = ?";
  $where[2][] = $is_new !== '0' ? "YEAR(C.appoint_date) = ?" : "YEAR(A.shipment_date) = ?";
  $where[3][] = "year_no = ?";
  $params[] = $year_no;
}
if ($month_no !== '0') {
  ...
  $where[2][] = $is_new !== '0' ? "MONTH(C.appoint_date) = ?" : "MONTH(A.shipment_date) = ?";
  $where[3][] = "month_no = ?";
  $params[] = $month_no;
}
if ($channel !== 'N') {
...
$data = [
  'appointment' => fetchAll($objCon, $sqlappoint, $params),
  'costSheets'  => fetchAll($objCon, $sqlcostsheet, $params),
  'orders'      => fetchAll($objCon, $sqlorder, $params),
  'revenue'     => fetchAll($objCon, $sqlrevenue, $params),
  'segments'    => fetchAll($objCon, $sqlsegment, $params),
  'regions'     => fetchAll($objCon, $sqlregion, $params)
];
